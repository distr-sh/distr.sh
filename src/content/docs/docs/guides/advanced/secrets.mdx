...
Secrets are scoped by organization. The scope determines which deployments can reference a secret and who can manage it.

### Vendor-global secrets

Vendor-global secrets are available to all your deployments across all customers. Use these for shared credentials that apply to your infrastructure regardless of which customer is using it — for example, a shared image registry token or a third-party API key used in every deployment.

To create or manage vendor-global secrets:

1. In the Vendor Portal sidebar, click **Secrets**.
2. Make sure no customer filter is active (the list should show **All** or your organization name).
3. Create, edit, or delete secrets from this view.

These secrets are available to any deployment configuration across all customer organizations.

### Customer-scoped secrets

Customer-scoped secrets are only available to deployments for a specific customer. Use these for credentials that belong to one customer — for example, a customer's own database password or an API key that is unique to their environment.

There are two ways to reach customer-scoped secrets as a vendor:

**Option A — from the Secrets page:**

1. In the Vendor Portal sidebar, click **Secrets**.
2. Use the customer filter dropdown to select a specific customer organization.
3. Secrets shown are scoped to that customer. Create, edit, or delete from this view.
...
3. This opens the same scoped view filtered to that customer.

Customer-scoped secrets are only injected into deployments for that customer. They are not accessible to other customers' deployments and do not appear in the vendor-global secret list.
...
<Aside type="tip">
  If a deployment references both a vendor-global secret and a customer-scoped
  secret with the same key name, the customer-scoped secret takes precedence.
</Aside>
