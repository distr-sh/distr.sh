...
Shared env variables are also a great way to share env variables between the services with delecaring them multiple times.
...
Altough I would like to see docker a jobs extension (similar to Secrets) it is not that complicated to implement peridoc jobs for Docker Compose.
